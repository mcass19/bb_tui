...
- **Follows the `positions` → `configurations` rename in `bb` core (breaking).** `bb` 0.27.0's multi-DoF joint work renamed `BB.Robot.Runtime.positions/1` to `configurations/1` — a joint's configuration is only a float when it has one degree of freedom — so the TUI crashed at startup (`UndefinedFunctionError` in `BB.TUI.App.init/1`) against `bb` >= 0.27. `BB.TUI.Robot.positions/2` is now `configurations/2` and routes to the new accessor both locally and over `:rpc`. The joints panel and the visualization drive single-DoF joints, so values remain floats and behaviour is otherwise unchanged; planar and floating joints surface transform structs the TUI does not yet render. The `mix.exs` requirement moves from `~> 0.20` to `~> 0.27` to encode the new API. Mirrors `bb_liveview` v0.3.0's migration.
